...
//			Q1.find all transactions in the year 2011 and sort them by value(asc)?
		/*
		 * testData().stream().filter((t)->{return
		 * t.getYear()==2011;}).sorted(Comparator.comparing(Transaction::getValue)).
		 * collect(Collectors.toList()).forEach(System.out::println);
		 */

//			Q2. what are all the unique cities where the traders work?
		testData().stream().map((t) -> t.getTrader().getCity()).collect(Collectors.toSet())
				.forEach(System.out::println);

//			Q3. find all traders from Cambridge and sort them by name?
		testData().stream().map(Transaction::getTrader).filter(t -> "Cambridge".equals(t.getCity()))
				.sorted(Comparator.comparing(Trader::getName)).collect(Collectors.toList())
				.forEach(System.out::println);

//			Q4. return a string of all the traders name sorted alphabetically?
		String collect = testData().stream().map(t -> t.getTrader().getName()).distinct().sorted()
...
//			Q6. print the values of all transaction from the traders living in Cambridge?
		testData().stream().filter(t -> "Cambridge".equals(t.getTrader().getCity())).map(Transaction::getValue)
				.collect(Collectors.toList()).forEach(System.out::println);
...
//			 Q8. find the transaction with the smallest value?
		...
		Transaction transaction3 = testData().stream().sorted(Comparator.comparing(Transaction::getValue)).findFirst()
				.get();
