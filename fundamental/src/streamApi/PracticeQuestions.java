...
//			 Q3. filter those students who studies in 3rd year and belongs to "Bihar"?
		Predicate<Student> p = (s) -> {
			return s.getStuYear() == 3 && s.getStuState().equals("Bihar");
		};
...
//			Q7. find the Student whose cgpa is maximum?
		Student student = students.stream().sorted(Comparator.comparing(Student::getCgpa)).collect(Collectors.toList())
				.get(students.size() - 1);
		System.out.println(student);
		Student student2 = students.stream().sorted(Comparator.comparing(Student::getCgpa).reversed())
...
//			 Q5. group Students by branch and their semester?
		Map<String, Map<Integer, List<Student>>> group = students.stream()
				.collect(Collectors.groupingBy(Student::getStuBranch, Collectors.groupingBy(Student::getStuSem)));
		/*
		 * for(Map.Entry<String, List<Student>> map:group.entrySet()) { for(Student
		 * s:map.getValue()) {
